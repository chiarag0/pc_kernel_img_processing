...
def plotFixedKernel(cpuDf, gpuDf):
    for bx, by in gpuDf[["blockX", "blockY"]].drop_duplicates().values:
        ...
        ax.set_xlabel("Risoluzione immagine")
        ax.set_ylabel("Speedup (CPU / GPU)")
        ax.set_title("Speedup vs Risoluzione")
        ...
        ax.set_title("Tempo GPU vs Risoluzione")
        ax.legend(fontsize=9)
        ax.grid(True, alpha=0.3)
        ax.set_yscale("log") 
    ...
    fig.suptitle("Fixed Kernel (11×11) — Best block size per variante",
                 fontsize=13, fontweight="bold")
    for variant in VARIANT_ORDER:
    ...
    ax.set_xlabel("Risoluzione immagine")
    ax.set_ylabel("Speedup (CPU / GPU)")
    ax.set_title("Speedup con best block size")
    ax.legend(fontsize=9)
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
...
def plotFixedImage(cpuDf, gpuDf):
    ...
    for bx, by in gpuDf[["blockX", "blockY"]].drop_duplicates().values:
        ...
        fig, axes = plt.subplots(1, 2, figsize=(14, 5))
        fig.suptitle(
            f"Fixed Image (1080p) — Block size {blockLabel}",
            fontsize=13, fontweight="bold"
        )
        ...
        ax.set_xlabel("Dimensione kernel (lato)")
        ax.set_ylabel("Speedup (CPU / GPU)")
        ...
        ax.set_xlabel("Dimensione kernel (lato)")
        ax.set_ylabel("Tempo (ms)")
        ax.set_title("Tempo GPU vs Kernel size")
        ax.legend(fontsize=9)
        ax.grid(True, alpha=0.3)
        ax.set_yscale("log")
        ax.set_xticks(kernelSizes)
    ...
    fig, ax = plt.subplots(figsize=(10, 5))
    fig.suptitle("Fixed Image (1080p) — Best block size per variante",
                 fontsize=13, fontweight="bold")
    for variant in VARIANT_ORDER:
        vData = gpuDf[gpuDf["variant"] == variant]
        bestB = vData.groupby(["blockX","blockY"])["speedup"].mean().idxmax()
    ...
    fig.suptitle("Fixed Image (1080p) — Impatto block size per variante",
                 fontsize=13, fontweight="bold")
...
if __name__ == "__main__":
    ...
    result = loadCsv("fixed_kernel.csv")
    if result:
        cpuDf, gpuDf = result
        print("-- fixed_kernel.csv --")
        plotFixedKernel(cpuDf, gpuDf)

    result = loadCsv("fixed_image.csv")
    if result:
        cpuDf, gpuDf = result
        print("\n-- fixed_image.csv --")
